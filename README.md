...
spectator mode and not be able to know any information about the current progress of the speedrun. They can neither move
and inspect the world nor spectate the current player. The order of turns is determined by the order of players joining
the server. The ordering would cycle.
...
**/relay pause**, resumed by **/relay resume**, and stopped by **/relay stop**. The time limit of a single turn in ticks
can be queried and set with **/relay countdown get** and **/relay countdown set**.